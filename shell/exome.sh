# GATK variant caller for whole exome NGS sequencing
...
if [ ! -f $p/step2.log ]; then
	...
	for c in ${chr[*]}; do
    		echo "$JAVA_GATK -T UnifiedGenotyper -glm BOTH  -R $ref $bams -o $vcf.$c.vcf $opt  -L $CaptureBed.$c.bed  -A AlleleBalance -A DepthOfCoverage -A FisherStrand
